...
//                System.out.println("导入成功！共处理 100 条记录");
            	Map<String,Object> r=this.doTotalToday(//excelFile, resourcePath, outImgPath, null
            			);
            	if(null!=r) {
            		SGDataHelper.getLog().print(
            				"今天导入的数据: minId "+SGDataHelper.ObjectToLong0(r.get("a"))
            				+", maxId "+SGDataHelper.ObjectToLong0(r.get("b"))
            		);
            	}else {
            		SGDataHelper.getLog().print("查询失败");
            	}
...
//    }
	...
	public Map<String,Object> doTotalToday() {
            ...
            query.Add("create_date",end,SqlExpressionOperator.Less);
				...
				SGSqlCommandString sql=new SGSqlCommandString(
						SGDataHelper.FormatString(
								"select min(resource_id) as a,max(resource_id) as b from {0} {1}",
								service.getTableName(resourceType),
								query.ToSql())
						);
				SGDataTable r2=dstExec.GetDataTable(sql.toString(), null);
				if(null!=r2) {
					return r2.ToDictList().get(0);
				}
