...
public class UncheckDatetimeConvert002 extends TestCase {
	private static final Logger LOGGER = LoggerFactory.getLogger(UncheckDatetimeConvert002.class);
...
//		System.out.println(new PFDate(cal).toString());
	...
	public void testTimestampLong2() throws Exception {
		...
		long dateLong = UncheckDatetimeConvert001.getTimestampLongFromApp("2022-09-09 18:00:00");
...
//		
	...
	public void testTimestampYearAtDifferentTimeZone01() throws Exception {
		...
		TimeZone timeZone = TimeZone.getTimeZone("GMT+6");
		TimeZone.setDefault(timeZone);
		Timestamp date = UncheckDatetimeConvert001.getTimestampFromSql2(
	...
	public void testLocalDateTimeToLong() throws Exception {
		initPFHelper();
		TimeZone timeZone = TimeZone.getTimeZone("GMT+8");//这里设置什么时区都不影响后面的输出,因为LocalDateTime初始化根本没用到这个时区信息
		LocalDateTime ldt= LocalDateTime.of(2022,12,6,5,0,0);
		System.out.println(ldt);//2022-12-06T05:00
		long timestamp = ldt.toInstant(ZoneOffset.of("+8")).toEpochMilli();
		System.out.println(timestamp);//1670274000000
		 timestamp = ldt.toInstant(ZoneOffset.of("+6")).toEpochMilli();
		System.out.println(timestamp);//1670281200000

		timestamp = ldt.atZone(TimeZone.getTimeZone("GMT+8").toZoneId()).toEpochSecond();
		System.out.println(timestamp);//1670274000
		timestamp = ldt.atZone(TimeZone.getTimeZone("GMT+6").toZoneId()).toEpochSecond();
		System.out.println(timestamp);//1670281200
	}
	public void testTimeZone() throws Exception {
		initPFHelper();
		System.out.println(TimeZone.getTimeZone("GMT+0").getRawOffset());//0
		System.out.println(TimeZone.getTimeZone("GMT+8").getRawOffset());//28800000
		System.out.println(ZoneOffset.of("+8"));//28800000
	}
}
